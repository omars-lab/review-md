...
function threadSearchText(t) {
  const parts = [t.id, anchorTypeLabel(t.anchor)];
...
function threadsDigest(files, opts = { scope: "vault" }) {
  ...
  for (const f of kept) {
    ...
    for (const t of f.threads) {
      ...
      for (const m of t.messages) lines.push("", `**${m.author}** \xB7 ${m.ts}`, "", m.body);
    }
  }
...
var SIDECAR = /^\.(.+)\.comments\.md$/;
var SKIP_DIRS = /* @__PURE__ */ new Set([".git", "node_modules", ".obsidian", ".trash"]);
...
function printJson(target, files) {
  const isDir = existsSync(target) && statSync(target).isDirectory();
  const out = isDir ? { root: target, files: files.map(({ file, path, uid, threads }) => ({ file, vaultPath: path, uid, threads })) } : { uid: files[0].uid, file: files[0].file, vaultPath: files[0].path, threads: files[0].threads };
